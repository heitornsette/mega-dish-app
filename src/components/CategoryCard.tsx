...
const CategoryCard = ({ icon: Icon, name, onClick, className }: CategoryCardProps) => {
  const isEmoji = typeof Icon === 'string';
  
  return (
    <Card
      onClick={onClick}
      className={`group cursor-pointer overflow-hidden border-border bg-card p-6 transition-all hover:shadow-lg hover:scale-105 hover:border-primary/50 ${className || ''}`}
    >
